...
public class ArmbandRenderer implements AccessoryRenderer {
    private final ResourceLocation left_texture;
    private final ResourceLocation right_texture;
    private final HumanoidModel<LivingEntity> model;

    public ArmbandRenderer(ModelLayerLocation layerLocation, ResourceLocation left_texture, ResourceLocation right_texture) {
    ...
    public <M extends LivingEntity> void render(
        ...
        if (stack.getItem() instanceof IDyable dyable) {
            ...
            VertexConsumer vertexBuilder = ItemRenderer.getFoilBuffer(
                multiBufferSource,
                RenderType.armorCutoutNoCull(reference.slot() == 0 ? this.left_texture : this.right_texture),
                false,
                stack.hasFoil()
            );
